...
import tomson124.hazardousresearch.container.HRContainer;
import tomson124.hazardousresearch.container.slot.SlotBase;
import tomson124.hazardousresearch.api.tile.IContainerProvider;
import tomson124.hazardousresearch.api.tile.IInventoryProvider;
import tomson124.hazardousresearch.network.PacketHandler;
...
public class TileEntityMachineBase extends TileEntity implements ITickable, IInventory, ISidedInventory {
    ...
    protected Optional<HRContainer> getContainerForTile() {
    ...
    public int[] getSlotsForFace(EnumFacing side) {
        if (getContainerForTile().isPresent()) {
            HRContainer container = getContainerForTile().get();
    ...
    public boolean canExtractItem(int index, ItemStack stack, EnumFacing direction) {
        if (getContainerForTile().isPresent()) {
            HRContainer container = getContainerForTile().get();
            if (container.slotMap.containsKey(index)) {
                SlotBase slot = container.slotMap.get(index);
